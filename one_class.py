...
def train(hyp, opt, device, tb_writer=None, wandb=None):
    ...
    assert len(names) == 1, '%g names found for nc=%g dataset in %s' % (len(names), nc, opt.data)  # check
    ...
    # Optimizer
    nbs = 16  # nominal batch size CHANGED TO 16
    ...
    if opt.adam:
        optimizer = optim.Adam(pg0, lr=0.0001, betas=(0.9, 0.999))  # CHANGED
    else:
